...
static inline cs_alpha_op *alpha_get_op(cs_insn *insn, int idx) {
...
static inline const char *alpha_get_op_regname(csh h, cs_insn *insn, int idx) {
	cs_alpha_op *op = alpha_get_op(insn, idx);
	if (!op || op->type != ALPHA_OP_REG) {
...
static inline st64 alpha_get_op_imm(cs_insn *insn, int idx) {
	cs_alpha_op *op = alpha_get_op(insn, idx);
	if (!op || op->type != ALPHA_OP_IMM) {
...
static void alpha_fillval(RzAnalysis *a, RzAnalysisOp *op, csh handle, cs_insn *insn) {
	if (!insn->detail) {
		return;
	}
	uint8_t srci = 0;
	cs_alpha *al = &insn->detail->alpha;
	if (!al) {
		return;
	}
	for (uint8_t i = 0; i < al->op_count; ++i) {
		...
		fill_from_alpha_op(a->reg, handle, av, alphaop);
...
static void alpha_opex(RzStrBuf *ptr, csh handle, cs_insn *p_insn) {
	if (!p_insn->detail) {
		return;
	}
	PJ *pj = pj_new();
	...
	pj_o(pj);
	pj_ka(pj, "operands");
	cs_alpha *al = &p_insn->detail->alpha;
...
static void alpha_op_set_type(RzAnalysisOp *op, csh h, cs_insn *insn) {
	switch (insn->id) {
	default: {
	...
	case Alpha_INS_BR:
		...
		op->jump = (ut32)alpha_get_op_imm(insn, 0);
		break;
	case Alpha_INS_BSR:
...
static int rz_analysis_alpha_op(RzAnalysis *a, RzAnalysisOp *op, ut64 addr, const ut8 *data, int len, RzAnalysisOpMask mask) {
	...
	alpha_op_set_type(op, handle, insn);

	if (mask & RZ_ANALYSIS_OP_MASK_OPEX) {
		alpha_opex(&op->opex, handle, insn);
	}
	if (mask & RZ_ANALYSIS_OP_MASK_VAL) {
		alpha_fillval(a, op, handle, insn);
	}
